...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments, data:list=None):
        ...
        rank0_print(f"Loaded {len(self.data_dict_list)} samples from {data_path}")
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                next_index = min(i + 1, len(self.data_dict_list) - 1)
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in self.data_dict_list[i]:
            data_dict["image"] = image
        elif "video" in self.data_dict_list[i]:
            data_dict["image"] = image
        ...
        data_dict["id"] = self.data_dict_list[i].get("id", i)
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args, data=None) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = LazySupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path, data_args=data_args, data=data)
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            model = LlavaLlamaForCausalLM.from_pretrained(
                ...
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
                low_cpu_mem_usage=True if training_args.bits<16 else False,
                **customized_kwargs,
            )
...
def train(attn_implementation=None):
    ...
    rank0_print(f"Try Prompt version: {model_args.version}")
    rank0_print(training_args.attn_implementation)
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args, data=future.result())
